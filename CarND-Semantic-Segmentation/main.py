...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Deconvolution
    deconv_1 = tf.layers.conv2d_transpose(conv_1x1_7, num_classes, 4, 2, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    # Skip layer 4
    skip_1 = tf.add(deconv_1, conv_1x1_4)
    deconv_2 = tf.layers.conv2d_transpose(skip_1, num_classes, 4, 2, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    # Skip layer 3
    skip_2 = tf.add(deconv_2, conv_1x1_3)
    output = tf.layers.conv2d_transpose(skip_2, num_classes, 16, 8, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
